...
def generate_economy_content():
    ...
    [썸네일 문구 및 프롬프트]:
    - 메인 타이틀: (핵심 키워드 중심의 직관적이고 굵은 텍스트, 예: 기준금리 인하 충격)
    - 서브 타이틀: (한눈에 들어오는 부제목 또는 경고/알림 문구, 예: 내 예금과 대출은 어떻게 될까?)
    - 이미지 생성 프롬프트: (신뢰감을 주는 깔끔한 금융/경제 인포그래픽 스타일 배경에 중앙/하단에 선명한 한국어 텍스트 레이아웃이 적용된 고품질 썸네일 생성을 위한 영문 프롬프트 작성)
    """
